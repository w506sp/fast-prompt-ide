...
*   **Workspace:** Kullanıcıların projelerini organize ettiği, ekip çalışmasına temel oluşturabilecek üst seviye alan.
...
*   **Execution:** Modelin her çalıştırılmasında oluşan kayıt. Girdiler, çıktılar, `latency_ms` (milisaniye cinsinden gecikme) ve `token_usage` (input/output token sayıları) bu tabloda tutularak performans analizi yapılmasına olanak tanır.
